...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    printf("Discovered %s\n", [[peripheral name] cStringUsingEncoding: NSUTF8StringEncoding]); // TODO: Remove printfs
    printf("  RSSI: %s\n", [[RSSI stringValue] cStringUsingEncoding: NSUTF8StringEncoding]);
    NSString* powerLevel = [[advertisementData valueForKey:@"kCBAdvDataTxPowerLevel"] description];
    id serviceDataDict = [advertisementData valueForKey:@"kCBAdvDataServiceData"];
    NSObject* serviceData =[[(NSDictionary*)serviceDataDict allValues] objectAtIndex:0];
    NSString* beaconId = [[NSString alloc] initWithData:(NSData*)serviceData encoding:NSUTF8StringEncoding];
    NSObject* name = [self formatName:peripheral.name];
    NSString* dataString = [NSString stringWithFormat:@"%@/%@/%@", beaconId, powerLevel, name];
    ...
    /*
     ...
     NSObject* theValue =[[(NSDictionary*)value allValues] objectAtIndex:0];
     id xx = [theValue class];
     beaconId = [[NSString alloc] initWithData:(NSData*)theValue encoding:NSUTF8StringEncoding];
     
     printf("   keyUUID: %s\n", [beaconId cStringUsingEncoding: NSUTF8StringEncoding]);
     
     }
     if ([key isKindOfClass:[CBUUID class]]) {
     CFStringRef str = CFUUIDCreateString(nil, (CFUUIDRef)key);
     NSString* res = (__bridge_transfer NSString*)str;
     printf("   CBUUID: %s\n", res);
     }
